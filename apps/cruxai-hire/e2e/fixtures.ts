...
export const test = base.extend<{ page: import('@playwright/test').Page }>({
  page: async ({ page }, use) => {
    ...
    // Catch-all abort registered first (lowest priority — LIFO means last registered wins)
    await page.route('**/api/**', (route) => {
      console.warn(`[TEST] Unmocked API request aborted: ${route.request().method()} ${route.request().url()}`);
      route.abort();
    });

    // Mock better-auth session endpoint — registered after catch-all so it takes priority
