...
const ALON = { name: 'Alon Noma', email: 'alon@noma.security', password: 'alon123crux' };

async function main() {
  let userId: string | undefined;
  try {
    const result = await auth.api.signUpEmail({ body: ALON });
    userId = result.user?.id;
    console.log(`✓ Created user: ${ALON.email}`, userId);
  } catch (e) {
    console.warn(`User may already exist, looking up by email…`, (e as Error).message);
  }

  if (!userId) {
    const existing = await db.select({ id: authUser.id }).from(authUser).where(eq(authUser.email, ALON.email)).limit(1);
    userId = existing[0]?.id;
  }

  if (!userId) {
    console.error('Could not resolve user id for', ALON.email);
    process.exit(1);
  }

  const questionId = toUuid(`q-monday-${ALON.email}`);
  ...
  console.log(`✓ Seeded Monday question (${questionId}) owned by ${ALON.email}`);

  const roleId = toUuid(`role-monday-${ALON.email}`);
