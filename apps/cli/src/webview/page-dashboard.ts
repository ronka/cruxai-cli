...
import { DateFilter, DailyActivity, PRACTICE_GROUPS, AntiPatternData, WorkflowOptimizationData, SkillTriageResult, CatalogDiscoverResult, CatalogTriageResult, GroupScore, CodeProductionData } from '@crux/core/types';
...
function renderDashboardMarkup(
  ...
  scores: import('@crux/core/types').GroupScore[],
...
function renderSkillResults(
  triaged: import('@crux/core/types').TriagedCluster[],
  clusters: import('@crux/core/types').WorkflowCluster[],
  catalogMatches: import('@crux/core/types').CatalogItem[],
): void {
...
async function loadDashSkills(currentFilter: DateFilter): Promise<void> {
  ...
  try {
    ...
    let triagedResults: import('@crux/core/types').TriagedCluster[] = [];
    let catalogMatches: import('@crux/core/types').CatalogItem[] = [];
