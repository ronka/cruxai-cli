...
/* The dashboard shell DOM (sidebar nav + content host). Shared by the VS Code
...
import { FF_TOKEN_REPORTING_ENABLED } from '@crux/core/constants';
...
export function getDashboardShellHtml(opts?: {
  ...
  <nav id="sidebar">
    <ul class="nav-links">
      ...
      ${includeContextHealth ? '<li><a href="#" data-page="config-health"><span class="nav-icon"><svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M8 1L1.5 4.5V11.5L8 15L14.5 11.5V4.5Z" stroke="currentColor" stroke-width="1.3" fill="none"/><path d="M8 5.5V10.5M5.5 8H10.5" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/></svg></span> Context Health</a></li>' : ''}
      ${includeLevelUp ? '<li><a href="#" data-page="level-up"><span class="nav-icon"><svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M8 1l2 5h5l-4 3.5 1.5 5L8 11.5 3.5 14.5 5 9.5 1 6h5z" stroke="currentColor" stroke-width="1.2" fill="none"/></svg></span> Level Up</a></li>' : ''}
    </ul>
    <div class="sidebar-filters">
      <div class="sidebar-filter">
        <label>Workspace</label>
        <div class="ws-toggle" id="ws-toggle">
          <button class="ws-toggle-btn${scanMode ? '' : ' active'}" data-ws="current">Current</button>
          <button class="ws-toggle-btn${scanMode ? ' active' : ''}" data-ws="all">All</button>
        </div>
